...
module psg #(parameter CLK_HZ = 32'd3_506_580, parameter REVERB = 1,
  ...
  wire         state_sample_read = prun && pph < 7'(PLOSC + PSG_SPAR);
...
`endif
  ...
  logic wi_neg;
  logic [PSG_NV-1:0] clr_ack;              // pairs with the sequencer's clr_tog
  ...
  logic [PSG_VADR-1:0] wlk_ra, wlk_wa;
  logic [15:0]         wlk_wd;
  always_comb begin
    if (pph < 7'(PLOSC))
  ...
  // The wave layer itself is u_wave (rtl/psg_wave.sv). It takes the
  // evaluation context and the live/old voice fields, and returns the three
  // values the walk consumes.
