...
module chip(input logic clk, input logic cpuclk, input logic reset,
  ...
  addressdecoder decoder(.addr, .rw(write), .cpu_di, .tb_do, .sp_do, .ram_do, .tb_cs, .sp_cs, .ram_cs);
  /* verilator lint_on UNOPTFLAT */

  // 8x64kbit Async RAM
  ram_async #(.A(16), .D(8), .FILE("./rtl/ram.hex")) ram(.clk(~clk), .cs(ram_cs), .rw(write), .addr(addr[15:0]), .di(cpu_do), .dout(ram_do));
  
  // Control Unit
  // control c0(.clk, .reset, .vsync, .addr, .data(cpu_do), .din(cpu_di), .rw);

  cpu6502 cpu0(.clk(cpuclk), .reset, .address(addr), .data_in(cpu_di), .data_out(cpu_do), .write(write));
  // cpu6502 cpu0(.clk, .reset, .AB(addr), .DI(cpu_di), .DO(cpu_do), .WE(~rw), .IRQ(0), .NMI(0), .RDY(1));

  // Text Video Buffer  
  logic [3:0] text_color;
  logic [RGB-1:0] trgb; 
  textbuffer tb(.clk(~clk), .reset, .addr(addr[9:0]), .cs(tb_cs), .rw(write), .di(cpu_do), .dout(tb_do), .hpos, .vpos, .vsync, .hsync, .color(text_color));
  palette #(.RED(RED), .GREEN(GREEN), .BLUE(BLUE), .FILE("./rtl/palette888.bin")) pal_text(.clk, .color(text_color), .rgb(trgb));

  // Video Sprites  
  logic pixel;
  logic [RGB-1:0] srgb;
  sprite s0(.clk(~clk), .reset, .addr(addr[3:0]), .cs(sp_cs), .rw(write), .di(cpu_do), .dout(sp_do), .hpos, .vpos, .hsync, .vsync, .pixel);
