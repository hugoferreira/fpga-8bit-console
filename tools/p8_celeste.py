...
Emits, all to src/celeste/:
    gfx.asm     sheet image, tile-id -> (slot base, attributes) tables
    rooms.asm   the resident rooms as tile ids, plus the tile flag table
...
def main():
    ...
    # --- gfx.asm ----------------------------------------------------------
    with open(os.path.join(args.out, "gfx.asm"), "w") as f:
        f.write(f"""; ------------------------------------------------------------------------------
...
""")
    ...
    with open(os.path.join(args.out, "rooms.asm"), "w") as f:
        f.write(f"""; ------------------------------------------------------------------------------
; GENERATED by tools/p8_celeste.py - do not edit.
;
...
""")
    ...
    print(f"wrote {args.out}/gfx.asm and {args.out}/rooms.asm")
