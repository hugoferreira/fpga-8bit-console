...
static void mangle_path(FILE *file, LaSlice owner, LaSlice path)
{
    ...
    while (cursor < end) {
        ...
        fputs("__", file);
        mangle_component(file, (LaSlice){cursor, (la_u16)(dot - cursor)});
                                     ...
                                     const LaEvent *event)
{
    ...
    fprintf(output->assembly, "__offset ; laasm %.*s.%.*s\n",
...
static int emit_indexed_operation(HostOutput *output, const LaEvent *event)
{
    ...
    fprintf(output->assembly, "    %s (%.*s), y ; laasm %.*s.%.*s\n",
            store ? "sta" : "lda",
            (int)event->base.length, event->base.data,
            (int)event->owner.length, event->owner.data,
            (int)event->path.length, event->path.data);
...
static int emit_frame_local(HostOutput *output, const LaEvent *event)
{
    ...
    fputs("    tsx\n", output->assembly);
    if (!begin_line(output, event->span, "frame-local")) return 0;
    fprintf(output->assembly, "    %s $%04x, x ; laasm local %.*s\n",
...
static int host_event(void *context, const LaEvent *event)
{
    ...
    case LA_EVENT_HEADER:
        ...
        if (emitted) {
            fprintf(output->assembly,
                    "; generated by laasm format %u, target %.*s/%u\n",
    ...
    case LA_EVENT_PROCEDURE_MEMBER:
        ...
        if (!emitted) break;
        fprintf(output->assembly, "; laasm member %.*s.%.*s : %.*s ",
                (int)event->owner.length, event->owner.data,
                (int)event->path.length, event->path.data,
                (int)event->aux.length, event->aux.data);
...
{
    fputs(
        "usage: laasm --target console6502 --output FILE --map FILE "
        "[OPTIONS] INPUT.la.asm\n"
        "\n"
        "Compile layout-aware assembly to customasm source and a JSON map.\n"
        "\n"
        "Options:\n"
        ...
        "\n"
        "Example:\n"
        "  laasm --target console6502 --output game.asm "
        "--map game.map.json game.la.asm\n"
        "\n"
...
{
    fprintf(file,
            "laasm 0.2 language-format=%u target-format=%u map-format=2\n",
                          ...
                          HostArtifacts *artifacts)
{
    ...
    if (!open_temporary(options->output_path, artifacts->assembly_path,
                        ...
                        &artifacts->assembly)) {
        fprintf(stderr, "laasm: cannot create %s: %s\n",
                             ...
                             HostArtifacts *artifacts)
{
    if (rename(artifacts->map_path, options->map_path) != 0) {
        fprintf(stderr, "laasm: cannot publish %s: %s\n",
                options->map_path, strerror(errno));
        return 0;
    }
    artifacts->map_path[0] = 0;
...
static int run_customasm(const char *output_path, HostOutput *output)
{
    ...
    if (snprintf(binary_path, sizeof(binary_path),
                 "%s/laasm-check.XXXXXX", temporary_directory) >=
        (int)sizeof(binary_path)) {
        fputs("laasm: temporary directory path is too long\n", stderr);
        return 0;
    }
    descriptor = mkstemp(binary_path);
    if (descriptor < 0) {
        fprintf(stderr, "laasm: cannot create validation output: %s\n",
                strerror(errno));
        return 0;
    }
    close(descriptor);
    if (!shell_quote(quoted_output, sizeof(quoted_output), output_path) ||
        ...
        fputs("laasm: generated path is too long for customasm wrapper\n",
              stderr);
        unlink(binary_path);
        return 0;
    }
    if (snprintf(command, sizeof(command),
        ...
        fputs("laasm: customasm command is too long\n", stderr);
        unlink(binary_path);
        return 0;
    }
    pipe = popen(command, "r");
    if (pipe == 0) {
        fprintf(stderr, "laasm: cannot run customasm: %s\n", strerror(errno));
        unlink(binary_path);
        return 0;
    }
...
{
    if (*index + 1 >= argc) {
        fprintf(stderr, "laasm: option %s requires a value\n", option);
        return 0;
    }
    ...
    *run = 0;
    ...
    for (index = 1; index < argc; ++index) {
        if (!positional_only &&
            ...
            fprintf(stderr, "laasm: unknown option: %s\n", argv[index]);
            usage(stderr);
            return 2;
        } else if (options->input_path == 0) {
            options->input_path = argv[index];
        } else {
            fputs("laasm: only one input file is supported\n", stderr);
            return 2;
        }
    }
    if (native) {
        fputs("laasm: error[native-output-deferred]: direct machine-code "
    ...
    if (strcmp(options->target_name, la_target_console6502.name) != 0) {
        fprintf(stderr, "laasm: unknown target '%s'; available: %s\n",
    ...
    if (strcmp(options->input_path, options->output_path) == 0 ||
        ...
        fputs("laasm: refusing to overwrite the input file\n", stderr);
        return 2;
    }
    if (strcmp(options->output_path, options->map_path) == 0) {
        fputs("laasm: output and map must be different files\n", stderr);
        return 2;
    }
    if (existing_directory(options->output_path) ||
        existing_directory(options->map_path)) {
        fputs("laasm: output and map destinations must be files\n", stderr);
        return 2;
    }
...
int main(int argc, char **argv)
{
    ...
    if (realpath(options.input_path, canonical_input) == 0) {
        fprintf(stderr, "laasm: cannot open %s: %s\n",
    ...
    if (root_slash == 0) {
        fputs("laasm: input path has no directory\n", stderr);
        return 2;
    }
    *root_slash = 0;
    ...
    if (!add_host_module(&host_modules, canonical_input,
        ...
        fprintf(stderr, "laasm: cannot load %s: %s\n",
                options.input_path, strerror(errno));
        return 2;
    }
    ...
    if (module_workspace.data == 0) {
        fputs("laasm: cannot allocate module workspace\n", stderr);
    ...
    if (workspace.data == 0) {
        fputs("laasm: cannot allocate host workspace\n", stderr);
        discard_artifacts(&artifacts);
        free(module_workspace.data);
        free_host_modules(&host_modules);
        return 2;
