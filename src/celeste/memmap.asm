...
; ------------------------------------------------------------------------------
    O_TYPE = 0       ; 0 = free slot
    O_SPR = 1       ; the cart's sprite number
    O_X = 2       ; signed integer pixels
    O_Y = 3
    O_SPDX = 4       ; 8.8 signed, lo then hi
    O_SPDY = 6
    O_REMX = 8       ; 8.8 sub-pixel remainder
    O_REMY = 10
    O_HBX = 12      ; hitbox
    O_HBY = 13
    O_HBW = 14
    O_HBH = 15
    O_FLIP = 16      ; bit0 x, bit1 y
    O_FLAGS = 17      ; bit0 collideable, bit1 solids
    O_STATE = 18
    O_DELAY = 19
    O_DJUMP = 20
    O_GRACE = 21
    O_JBUF = 22
    O_DASHT = 23      ; dash_time
    O_DASHE = 24      ; dash_effect_time
    O_PBITS = 25      ; bit0 p_jump, bit1 p_dash, bit2 was_on_ground
    O_SPROFF = 26      ; animation, in quarter-frames
    O_DTX = 27      ; dash_target x, 8.8
    O_DTY = 29
    O_DAX = 31      ; dash_accel x, 8.8
    O_DAY = 33
    O_TGTX = 35      ; player_spawn's landing target
    O_TGTY = 36
    O_HAIR = 37      ; 5 nodes x {x lo,hi, y lo,hi} = 20 bytes
    O_SIZE = 64
    OBJ_MAX = 16
    HAIR_NODES = 5
...
; ------------------------------------------------------------------------------
    OBJPOOL = $5000   ; OBJ_MAX * O_SIZE = 1024
