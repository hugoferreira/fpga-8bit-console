; Vendored from customasm's std/cpu/6502.asm (hlorenzi/customasm, Apache-2.0),
; which covers all 151 documented NMOS 6502 opcodes and addressing modes.
; The cpu6502_accum_shorthand ruledef below is this project's addition: the
; corpus (written for ca65) omits the explicit `a` operand on asl/lsr/rol/ror.
...
#ruledef cpu6502_accum_shorthand
{
	asl => 0x0a
	lsr => 0x4a
	rol => 0x2a
...
; ca65's `lda #<label` / `lda #>label` idiom (load the low/high byte of a
